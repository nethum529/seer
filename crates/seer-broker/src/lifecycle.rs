...
mod tests {
    ...
    use super::{Lifecycle, RuntimeState, bound_reason, valid_generation};
    use crate::test_support::{remove_directory, temporary_directory};

    #[test]
    fn bounds_reasons_at_a_utf8_boundary() {
        let reason = format!("{}é", "x".repeat(511));
        assert_eq!(bound_reason(&reason).len(), 511);
        assert_eq!(bound_reason(&reason), "x".repeat(511));
    }

    #[test]
    fn rejects_invalid_persisted_records() {
        let directory = temporary_directory("lifecycle-invalid");
        let lifecycle = Lifecycle::new(&directory).expect("lifecycle must open");
        let record_path = directory.join("runtime-records/alice.json");
        fs::write(
            record_path,
            r#"{"version":2,"user_id":"alice","generation":"bad","state":"running","state_dir":"/tmp/other","last_reason":null}"#,
        )
        .expect("record must write");
        assert!(lifecycle.load("alice").is_err());
        remove_directory(&directory, "temporary directory must be removed");
    }

    #[test]
    fn records_generation_guarded_transitions() {
        let directory = temporary_directory("lifecycle-generation");
        let lifecycle = Lifecycle::new(&directory).expect("lifecycle must open");
        let starting = lifecycle
            .starting("alice")
            .expect("start record must write");
        assert_eq!(starting.state, RuntimeState::Starting);
        assert!(
            !lifecycle
                .mark_running_if_current("alice", "00000000000000000000000000000000")
                .expect("stale generation must check")
        );
        assert!(
            lifecycle
                .mark_running_if_current("alice", &starting.generation)
                .expect("running transition must write")
        );
        assert!(
            !lifecycle
                .mark_failed_if_current("alice", "00000000000000000000000000000000", "stale")
                .expect("stale failure must check")
        );
        remove_directory(&directory, "temporary directory must be removed");
    }

    #[test]
    fn validates_generation_shape() {
