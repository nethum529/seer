...
fn runtime_directory_path(xdg_runtime_dir: Option<OsString>, uid: u32) -> PathBuf {
    match xdg_runtime_dir.filter(|value| !value.is_empty()) {
        Some(root) => PathBuf::from(root).join("mux"),
        None => PathBuf::from(format!("/tmp/mux-{uid}")),
    }
}
...
fn runtime_binary() -> io::Result<PathBuf> {
    match env::var_os("MUX_RUNTIME_BIN") {
...
fn runtime_binary_next_to(executable: &Path) -> io::Result<PathBuf> {
    executable
        .parent()
        .map(|directory| directory.join(format!("mux-runtime{}", env::consts::EXE_SUFFIX)))
...
mod tests {
    ...
    fn selects_runtime_directories_and_private_mode() {
        ...
        assert_eq!(xdg, xdg_root.join("mux"));
        assert_eq!(fallback, Path::new("/tmp/mux-123"));
    ...
    fn finds_runtime_next_to_broker() {
        let path = runtime_binary_next_to(Path::new("/opt/mux/mux-broker"))
            .expect("runtime path must resolve");

        assert_eq!(path, Path::new("/opt/mux/mux-runtime"));
    }
